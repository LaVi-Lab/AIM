...
class CLIPVisionTower(nn.Module):
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            image_features = self.feature_select(image_forward_outs).to(images.dtype)
            desired_layer_k = outputs["desired_k"]
            desired_layer_k = desired_layer_k[:, 1:, :]
            hook_handle_k.remove()

            # 43.75% merging
            image_features = bipartite_soft_matching_merge(metric=desired_layer_k, r=288, x=image_features) # 50% kept
            desired_layer_k = bipartite_soft_matching_merge(metric=desired_layer_k, r=288, x=desired_layer_k) # 50% kept
            image_features = bipartite_soft_matching_merge(metric=desired_layer_k, r=36, x=image_features) # 43.75% kept
            desired_layer_k = bipartite_soft_matching_merge(metric=desired_layer_k, r=36, x=desired_layer_k) # 43.75% kept
            
            # 12.5% merging
            # image_features = bipartite_soft_matching_merge(metric=desired_layer_k, r=288, x=image_features) # 50% kept
            # desired_layer_k = bipartite_soft_matching_merge(metric=desired_layer_k, r=288, x=desired_layer_k) # 50% kept
            # image_features = bipartite_soft_matching_merge(metric=desired_layer_k, r=144, x=image_features) # 25% kept
            # desired_layer_k = bipartite_soft_matching_merge(metric=desired_layer_k, r=144, x=desired_layer_k) # 25% kept
            # image_features = bipartite_soft_matching_merge(metric=desired_layer_k, r=72, x=image_features) # 12% kept
            # desired_layer_k = bipartite_soft_matching_merge(metric=desired_layer_k, r=72, x=desired_layer_k) # 12% kept
            ############### AIM ###############
